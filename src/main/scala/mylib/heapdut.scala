  ...
  def main(srgs: Array[String]) {
    ...
    SimConfig.withWave.doSim(new heap(config,false)){dut =>
      ...
      while(idx<65536) {
        ...
        dut.io.now #= idx
        if(!dut.io.insert.valid.toBoolean && Random.nextInt(100)<4) {
          val insertD = idx + 256 + Random.nextInt(256) 
...
object MyHeapSim {
  ...
  def main(args: Array[String]) {
    ...
    val config = HeapConfig(8,HeapItemConfig(24,12),16,-1)
    val f = SignalFormat()
    f.add("idx",5)
    f.add("state",4)
    f.add("ready",3)
